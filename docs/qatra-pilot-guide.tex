...
\usepackage{xcolor}
\usepackage{booktabs}
% Ahead of tabularx, which loads array itself — placed after it, this pin
% does nothing. bidi (via polyglossia) patches tabular with a call to
% \\UseMathForPositioningText, which only array's 2024-06-01 rollback still
% defines. Without it every table fails with 'Undefined control sequence'.
...
% polyglossia, not just an Arabic font: XeTeX shapes Arabic letters
% correctly on its own but does not reorder words, so a multi-word phrase
% in a left-to-right paragraph comes out backwards. The scripts below are
...
% Upright, always. The quote boxes are italic and Segoe UI's italic face
% carries no Arabic, so the letters came out as empty boxes. Arabic has no
% italic tradition either — slanting it would be wrong even if the glyphs
% existed.
\newcommand{\ar}[1]{{\upshape\textarabic{#1}}}
...
with a telephone number. Nearest to Draria: CTS CHU Dou\'era 023 12 46 35, CTS
CHU Beni Messous 023 11 32 23, CTS CHU Mustapha 021 23 66 64. \\
...
% ===========================================================================
\section{How much to say}

The worry is natural and worth answering honestly: what stops somebody taking
the idea?

\subsection{The idea is not the asset}

Four applications already match blood donors in Algeria and one has ten
thousand installs. Nobody needs to steal the concept of connecting a donor to a
request --- it is already taken, four times over, by products that do it badly.

What none of them has is the consent record with the version of the text shown,
the reveal log nobody can delete, the compatibility table checked from both
directions, the vouching model, and the assertions that hold all of it in place.
That is months of work, and it cannot leave a room inside somebody's memory.

\subsection{And the repository is public}

Worth knowing before a meeting rather than during one: the code, this document
and the technical dossier can all be downloaded by anyone, without an account.
That was the right choice --- an argument nobody can check is a brochure --- but it
means secrecy is not what is protecting this work. The work is.

\subsection{The line that does exist}

\begin{center}
\begin{tabularx}{\textwidth}{@{}XX@{}}
\toprule
\textbf{Say freely} & \textbf{Never hand over} \\
\midrule
What it does and who it is for & Database credentials or admin access \\
That it is built, and that nobody uses it yet & Any donor's name, number or blood type \\
How consent and the reveal log work & A copy of the database \\
That you are looking for one pilot & Anything you have not read, signed \\
\bottomrule
\end{tabularx}
\end{center}

\subsection{``What exactly is it?''}

Not vague --- scoped. Nobody standing at a folding table wants the schema, for
the same reason a patient does not want pharmacokinetics. Two sentences, both
entirely true, and both leaving them somewhere to ask a second question from.

\begin{say}
``C'est une application qui aide les comit\'es \`a joindre les bons donneurs au bon
moment --- le bon groupe sanguin, dans la m\^eme wilaya, et seulement ceux qui sont
\'eligibles ce jour-l\`a. Le comit\'e garde sa liste, et les num\'eros restent
prot\'eg\'es.''
\end{say}

\begin{say}
\ar{تطبيق يساعد اللجان على الوصول إلى المتبرع المناسب في الوقت المناسب: الفصيلة المطابقة، في نفس الولاية، ومن هو مؤهل للتبرع اليوم. القائمة تبقى عند اللجنة، والأرقام محمية.}
\end{say}

\subsection{``Does it actually work?''}

There is a temptation to answer no --- to call it a prototype that is not
finished, because it lowers the stakes and explains why you are asking
questions. Resist it, for two reasons.

It is not true. It works, it is deployed, and you are carrying it. If you say it
does not and then hold up a working screen ten minutes later, you have
contradicted yourself in front of the one audience whose trust the entire
project depends on --- and every other careful thing you said becomes worth
less.

And it is the weaker ask. Nobody gives an afternoon to something that does not
exist yet. \emph{``It works, here it is, will you try it''} is a smaller request
than it sounds, and a far better one.

The honest answer keeps everything you wanted from the modest one:

\begin{say}
``Oui, elle fonctionne --- mais personne ne l'utilise encore. C'est justement
pour \c{c}a que je viens vous voir : je pr\'ef\`ere savoir ce dont vous avez besoin
avant d'aller plus loin.''
\end{say}

\begin{say}
\ar{نعم، التطبيق يعمل — لكن لا أحد يستعمله بعد. لهذا جئت أسألكم: أفضّل أن أعرف ما تحتاجونه قبل أن أكمل.}
\end{say}

\note{Humble and accurate at once. ``Nobody uses it yet'' is the honest weakness
and it is also the invitation --- it is precisely what you are asking them to
change.}
...
% ===========================================================================
\section{Three calls to make this week}

Everything up to here is preparation. These are the calls, written out, because
the hard part is not knowing what to say --- it is saying it the first time.

Each one asks a small, real question. None of them opens by pitching.

\subsection{Call one: your nearest transfusion centre}

CTS CHU Dou\'era, \textbf{023 12 46 35}. A few minutes from Draria, and they know
which associations collect there.

\begin{say}
``Bonjour, je suis \'etudiant \`a Draria. Je travaille sur un projet autour du don
de sang et je voudrais savoir quelles associations organisent des collectes chez
vous, et s'il y en a une pr\'evue prochainement. Est-ce que vous pourriez
m'orienter ?''
\end{say}

\begin{say}
\ar{السلام عليكم. أنا طالب من الدرارية، أعمل على مشروع حول التبرع بالدم. أردت أن أسأل: ما هي الجمعيات التي تنظّم حملات التبرع عندكم، وهل هناك حملة قادمة قريبًا؟}
\end{say}

\note{You are not selling anything on this call. You are finding out who the
local associations are --- which is the list you actually need, and which is not
published anywhere.}

\subsection{Call two: the ANS}

\textbf{023 53 32 79}. Tixeraine, Bir Khadem --- a few kilometres from you, but
call before going.

\begin{say}
``Bonjour, je suis \'etudiant \`a Draria. Je cherche le programme des collectes
mobiles pour la wilaya d'Alger. Le site affiche encore l'ann\'ee 2025 --- est-ce
qu'il existe une version \`a jour, ou une page o\`u les collectes sont annonc\'ees ?''
\end{say}

\note{A question they can answer in one sentence, about a page they publish
themselves. If the conversation opens up, then say what you are building --- not
before.}

\subsection{Call three: a mosque or a local association}

You already know one works: a collection ran in Baba Hassen during Ramadan,
after the tarawih. Mosque collections recur, and Ramadan and the summer are
precisely when the ANS says donations fall.

\begin{say}
\ar{السلام عليكم ورحمة الله. أنا طالب من الدرارية. رأيت حملة للتبرع بالدم هنا في رمضان، وقد طوّرت تطبيقًا يساعد على الوصول إلى المتبرعين حين تُطلب فصيلة معيّنة.}

\ar{طلبي بسيط: هل يمكنني وضع رمز استجابة سريعة على الطاولة في الحملة القادمة؟ من يمسحه يصبح ضمن قائمة يمكنكم الاتصال بها عند الحاجة، والقائمة تبقى عندكم.}
\end{say}

\begin{say}
``Je suis \'etudiant \`a Draria. J'ai vu qu'une collecte a eu lieu ici pendant le
Ramadan. J'ai d\'evelopp\'e une application qui aide \`a joindre les donneurs quand
un groupe sanguin pr\'ecis est n\'ecessaire. Est-ce que je peux poser un QR code
sur la table lors de la prochaine collecte ? Ceux qui le scannent deviennent une
liste que vous pouvez appeler --- et la liste reste chez vous.''
\end{say}

\subsection{If they ask what it costs}

\begin{say}
``Rien. Ni pour vous, ni pour les donneurs. Le don de sang est b\'en\'evole et le
restera --- c'est la loi, et c'est aussi le principe de l'application.''
\end{say}

% ===========================================================================
\section{What to ask Dou\'era}

One principle carries this whole call, and it protects the idea by accident:
\textbf{ask how they work now, never about your solution.}

Somebody describing their own week is relaxed and detailed. Somebody being asked
to evaluate a stranger's app is guarded, polite, and useless. And a question
like \emph{``would you use an application that\ldots''} teaches them your idea
while teaching you nothing --- they will say yes to be kind, and you will have
given away the plan in exchange for a lie.

Everything below asks about their work. None of it mentions software.

\subsection{If they ask why you want to know}

\begin{say}
``Je fais une \'etude sur le don de sang pour un projet universitaire. J'essaie de
comprendre comment \c{c}a se passe r\'eellement --- pas ce qui est \'ecrit dans les
documents.''
\end{say}

True, modest, and it makes them the expert. People help students. Very few
people help salesmen.

\subsection{The questions, in this order}

Start wide and harmless. The valuable ones are in the middle, and by then they
are talking.
...
& \textbf{Ask} & \textbf{Why} \\
\midrule
1 & ``Vous faites des collectes mobiles, ou seulement sur place ?'' &
Warm-up \\
2 & ``Quelles associations organisent des collectes chez vous ?'' &
\textbf{The list} \\
3 & ``Il y en a une pr\'evue prochainement ?'' &
A date \\
4 & ``Quand il vous manque un groupe rare en urgence, vous faites comment ?'' &
\textbf{The heart} \\
5 & ``Vous avez une liste de donneurs que vous pouvez rappeler ?'' &
Does it exist \\
6 & ``Vous les contactez comment --- t\'el\'ephone, Facebook ?'' &
The channel \\
7 & ``\c{C}a arrive souvent qu'un groupe manque ?'' &
Frequency \\
8 & ``Les donneurs reviennent, ou c'est surtout des premi\`eres fois ?'' &
The 22\% \\
9 & ``Qui s'occupe des associations chez vous ?'' &
A name \\
10 & ``Je peux vous rappeler si j'ai d'autres questions ?'' &
A second call \\
\bottomrule
\end{tabularx}
\end{center}

\subsection{Question four is the one}

\begin{say}
``Quand il vous manque un groupe rare en urgence, vous faites comment ?''
\end{say}

\begin{say}
\ar{عندما تنقصكم فصيلة نادرة بشكل عاجل، كيف تتصرفون؟}
\end{say}

Everything this project assumes is contained in the answer to that one
sentence, and you did not put any of it in their mouth. If they describe
telephoning down a list for two hours, or posting on Facebook and waiting, you
have just confirmed --- from a transfusion centre, in their own words --- the
problem the dossier argues from national statistics.

Say nothing after they answer. Let the silence sit. People fill it with the
part they were not going to tell you.

\subsection{Question eight quietly checks the ANS}

The agency says only 22\% of Algerian donors give regularly and calls that
figure \emph{``relativement faible''}. That number is the foundation of the whole
argument. Asking a centre whether their donors come back tests it against one
real place --- and if Dou\'era says most people give once and vanish, you are no
longer quoting a statistic, you are reporting something you were told.

\subsection{What not to ask}

\begin{itemize}[leftmargin=1.4em]
  \item \textbf{``Est-ce que vous utiliseriez une application qui\ldots''} ---
        teaches them the idea, and buys a polite yes that means nothing. Never
        ask anybody to predict their own behaviour.
  \item \textbf{Anything that sounds like criticism.} ``Pourquoi vous n'avez pas
        de syst\`eme ?'' makes them defend the centre instead of describing it.
  \item \textbf{For any donor's details.} Not a name, not a number. Asking would
        undo the one thing that makes this project different, on the first call.
  \item \textbf{For a decision.} This call ends with information and possibly a
        name. It does not end with a yes, and reaching for one costs you the
        second call.
\end{itemize}

\subsection{Write it down while they talk}

Five things, and the third is worth more than the rest together.

\begin{enumerate}[leftmargin=1.4em]
  \item Every association named --- this is the list that exists nowhere online.
  \item The date of the next collection, if there is one.
  \item \textbf{Their exact words} for what they do when a type is missing.
        Write the sentence, not a summary of it. A line quoted from a
        transfusion centre is worth more in front of a panel than any figure in
        the dossier, because every other team will have figures and none of
        them will have spoken to anyone.
  \item Whether donors come back, in their answer to question eight.
  \item The name of whoever you spoke to, and the name they gave you for
        associations.
\end{enumerate}

\note{If the call gives you nothing but item three, it has paid for itself.}
