...
\usepackage{xcolor}
\usepackage{booktabs}
% Pinned, and it has to be.
%
% bidi -- pulled in by polyglossia for the Arabic -- patches tabular through
% array-xetex-bidi.def, and that patch calls \\UseMathForPositioningText.
% Current array no longer defines it; only the dated 2024-06-01 rollback does.
% Without this line every table in the document fails with 'Undefined control
% sequence' at \\end{tabularx}, and the build stops after two pages.
%
% tabularx loads array itself, so the pin must come first.
...
\vfill
{\color{qatraGrey}Yacine Omar Fodhil \\ 9 September 2026}
\end{titlepage}
...
except the SangDz figures, re-checked against that operator's own statistics
page on 9 September 2026.}
...
\ar{بنك الدم الجزائري} (SangDz) & 1K+ & — & \textbf{Web registry of 7\,498
donors} — see below. One operator, Mostaganem. \\
Sadaka \ar{صدقة} & 1K+ & — & — \\
...
That number counts installs, and installs are the wrong measure to rest a claim
on. SangDz reports \textbf{7\,498 registered donors} on its website — roughly
seven times its install count, because the registry is the site, not the app.
Anyone who opens that page during a meeting will see it, so this dossier states
it first.

The correction sharpens the claim rather than weakening it. The field is not
empty of donors; somebody has already collected several thousand of them. It is
empty of the thing that turns a donor into a transfusion: not one product in the
table verifies a request, and not one closes the loop between a family asking
and a donor arriving. The gap is not the list. It is everything after the list.
...
\subsection{SangDz: the one competitor with real people in it}
\label{sec:sangdz}

Nabdz is empty and the market leader is a portfolio item. SangDz is neither, and
it deserves the section the others do not.

It is a \textbf{donor registry}. The product is one control — choose a blood
group, then wilaya, daïra, commune, and search. Its own statistics page reports
7\,498 registered donors, weighted as Algeria is: 2\,589 O+, 1\,854 A+, and 94
AB$-$. Nuxt, French and Arabic, with an Android application alongside. Its About
page is candid about what stands behind it: \emph{``Le site n'est affilié à
aucune organisation, association ou autre entité''} — no affiliation with any
organisation or association — and it is run by one person.

\textbf{This is a real asset and should be described as one.} Several thousand
donors is the half of a two-sided product that code cannot produce. Qatra does
not have it, and no amount of engineering substitutes for it.

But a registry answers one question, and it is not the question a family asks at
two in the morning. Four gaps follow from the model, not from the execution:

\begin{enumerate}[leftmargin=1.4em]
  \item \textbf{Nobody vouches for anything.} A registry lists people who once
        said yes. It has no request to verify, so it cannot verify one. Qatra's
        association vouching exists precisely at the point a registry has no
        opinion.
  \item \textbf{There is no request, so there is no loop.} Nothing is posted,
        nothing is answered, and no family ever learns who is coming. The search
        ends at a name and a number; what happens next is a phone call the
        platform never sees and cannot count.
  \item \textbf{A searchable directory of blood types is the harder position
        under Loi 18-07.} Blood type is health data. Qatra masks a number until
        it is deliberately opened, logs every opening in a record the donor can
        read, and records consent per purpose with the version of the text
        shown. A directory built to be searched has made the opposite choice,
        and it made it before the question was being asked.
  \item \textbf{One operator, no institution.} SangDz says so itself. Qatra's
        route runs through the associations that already organise donation, so
        the two are not competing for the same partner — which also means the
        7\,498 are not defended by anybody's mandate.
\end{enumerate}

\textbf{Where it is ahead, and this dossier says so plainly:} SangDz searches to
\textbf{commune}. Qatra stores wilaya alone. In a country where one wilaya can
be the size of a small nation, ``a donor in your commune'' is a better answer
than ``a donor in your wilaya'' to the only question being asked. That is a
schema decision, not a strategy, and Section~\ref{sec:notbuilt} carries it.
...
\none & Pagination beyond the 200-request caps, national and per wilaya \\
\none & \textbf{Location below wilaya.} SangDz searches to daïra and commune
(Section~\ref{sec:sangdz}); Qatra stores the wilaya alone \\
...
This design rested on an assumption --- that associations are the right partner
for reaching donors --- and the assumption turns out to be the agency's own
published practice. The ANS maintains a page naming the associations it
collects blood with: the F\'ed\'eration Alg\'erienne des Donneurs de Sang, whose
wilaya committees exist for precisely this purpose, the Scouts Musulmans
Alg\'eriens, the Croissant-Rouge Alg\'erien, and a dozen others including Irchad
wa Islah, the Rotary and Lions clubs and the UNJA. Several of those
partnerships carry dates on the page and run back to 2015.

An association hosting a collection is therefore ordinary rather than
exceptional. What Qatra adds is not the partnership --- that already exists ---
but memory: which donors, which types, and the date each becomes eligible
again.

\note{The same site publishes a national directory of 165 transfusion
structures across all 58 wilayas with telephone numbers, and a per-wilaya
mobile-collection programme. Both are reachable over \texttt{http}; the
\texttt{https} certificate had expired when this was checked.}
...
\begin{itemize}[leftmargin=1.4em]\small
  ...
  \item ANS association partnerships:
        \url{http://www.ans.dz/associations-donneur-sang/}
  \item ANS national directory of fixed collection centres:
        \url{http://www.ans.dz/centres-collectes-fixes/}
  \item ANS mobile collection programme:
        \url{http://www.ans.dz/programme-de-collectes-mobiles/}
\end{itemize}
