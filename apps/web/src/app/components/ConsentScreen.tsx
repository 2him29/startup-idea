...
import { recordConsent } from "@weare/core";
...
export function ConsentScreen({ onBack, onConsented }: ConsentScreenProps) {
  ...
  const points = [
    { icon: HeartPulse, text: t.consentPoint1, bg: "#FFECEC", fg: "#E5484D" },
    { icon: Lock, text: t.consentPoint2, bg: "#E4F6FB", fg: "#0E8BA8" },
    { icon: ShieldCheck, text: t.consentPoint3, bg: "#EAF6EF", fg: "#0E7A4B" },
  ];
          ...
          {points.map((point, i) => {
            ...
            return (
              <div key={i} className="flex items-start gap-3">
                ...
                <span className="text-[13.5px] font-semibold flex-1" style={{ color: "#0B2432", textAlign: "start" }}>{point.text}</span>
              </div>
            );
          })}
        </div>
